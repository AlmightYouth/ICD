...
class GenerationMixin:
    ...
    def generate(
        ...
        evil_input_ids: Optional[torch.Tensor] = None,
        ...
        **kwargs,
        ...
        if generation_mode == GenerationMode.GREEDY_SEARCH:
            ...
            if contrastive_decoding:  # Greedy CD
                assert student_model is not None or evil_input_ids is not None  # at least one mode is supported
                if student_model is not None:
                    ...
                    return self.prompt_contrastive_greedy_decode(
                        input_ids,
                        evil_input_ids=evil_input_ids,
                        logits_processor=logits_processor,
                        stopping_criteria=stopping_criteria,
                        pad_token_id=generation_config.pad_token_id,
                        eos_token_id=generation_config.eos_token_id,
            ...
            elif dola_decoding:  # Greedy DOLA
            ...
            elif contrastive_decoding:  # CD sample
                assert student_model is not None or evil_input_ids is not None  # at least one mode is supported
                if student_model is not None:
    ...
    def relative_top_filter(scores: torch.FloatTensor, relative_top: float = 0.1, filter_value: float = -float("Inf"), min_tokens_to_keep: int = 1) -> torch.FloatTensor:
        """Adaptive Plausibility Constraint in CD, only perform CD at low-confidence positions, in order to alleviate FP (reward implausible tokens) and FN (penalize plausible tokens).

        Args:
            scores (torch.FloatTensor): _description_
            relative_top (float, optional): _description_. Defaults to 0.1.
            filter_value (float, optional): _description_. Defaults to -float("Inf").
            min_tokens_to_keep (int, optional): _description_. Defaults to 1.
    ...
    @staticmethod
    def relative_top_filter_probs(scores: torch.FloatTensor, relative_top: float = 0.1, filter_value: float = 0.0, min_tokens_to_keep: int = 1) -> torch.FloatTensor:
        """Adaptive Plausibility Constraint in CD, only perform CD at low-confidence positions, in order to alleviate FP (reward implausible tokens) and FN (penalize plausible tokens).
    ...
    def contrastive_greedy_decode(
        ...
        **model_kwargs,
        ...
        if debug:
            ...
            tokenizer = AutoTokenizer.from_pretrained("/apdcephfs/share_916081/effidit_shared_data/yafuli/Codes/Hallucination/memorization/LLaMA-Efficient-Tuning/llama-2-7b-chat")
        while True:
            if synced_gpus:
                # Under synced_gpus the `forward` call must continue until all gpus complete their sequence.
                # The following logic allows an early break if all peers finished generating their sequence
            ...
            if not v2:
                ...
                base_probs = base_logits.softmax(dim=-1)
                if debug:
                    print("The Top-10 probabilities from the student model:")
                    print("Indices:", tokenizer.batch_decode(base_probs.topk(10).indices.squeeze(0).unsqueeze(-1)))
                    print("Values:", base_probs.topk(10).values.squeeze(0).unsqueeze(-1))
                final_logits = outputs[0][:, -1, :]
                final_probs = final_logits.softmax(dim=-1)
                if debug:
                    print("The Top-10 probabilities from the original model:")
                    print("Indices:", tokenizer.batch_decode(final_probs.topk(10).indices.squeeze(0).unsqueeze(-1)))
                    print("Values:", final_probs.topk(10).values.squeeze(0).unsqueeze(-1))
                if relative_top > 0.0:
                    final_logits = self.relative_top_filter(final_logits, relative_top)
                    base_logits = base_logits.log_softmax(dim=-1)
                    mask = final_logits[0] < -1e3
                ...
                logits = final_logits - base_logits
                if debug:
                    print(f"The Top-10 log-probabilities after contrasting (relative top {relative_top}):")
                    print("Indices:", tokenizer.batch_decode(logits.topk(10).indices.squeeze(0).unsqueeze(-1)))
                    print("Values:", logits.topk(10).values.squeeze(0).unsqueeze(-1))
                ...
                diff_probs = torch.nn.functional.relu(final_probs - base_probs)
                # diff_probs = final_probs - base_probs 
                final_probs = (final_probs + diff_probs).softmax(dim=-1)
                if debug:
                    print(f"The Top-10 probabilities after contrasting (relative top {relative_top}):")
                    print("Indices:", tokenizer.batch_decode(final_probs.topk(10).indices.squeeze(0).unsqueeze(-1)))
                    print("Values:", final_probs.topk(10).values.squeeze(0).unsqueeze(-1))
                    import ipdb; ipdb.set_trace()
                # next_token_logits = logits
                next_tokens_scores = final_probs
    ...
    def prompt_contrastive_greedy_decode(
        self,
        input_ids: torch.LongTensor,
        evil_input_ids: torch.LongTensor,
        relative_top: float = 0.1,
        logits_processor: Optional[LogitsProcessorList] = None,
        stopping_criteria: Optional[StoppingCriteriaList] = None,
        max_length: Optional[int] = None,
        ...
        **model_kwargs,
        ...
        evil_model_kwargs = copy.deepcopy(model_kwargs)
        del evil_model_kwargs["attention_mask"]
        ...
        while True:
            ...
            # prepare model inputs
            model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs)
            model_evil_inputs = self.prepare_inputs_for_generation(evil_input_ids, **evil_model_kwargs)
            ...
            evil_model_kwargs = self._update_model_kwargs_for_generation(
                evil_outputs, evil_model_kwargs, is_encoder_decoder=self.config.is_encoder_decoder
            )
    ...
    def prompt_contrastive_decoding_sample(
        self,
        input_ids: torch.LongTensor,
        evil_input_ids: torch.LongTensor,
        relative_top: float = 0.1,
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # prepare model inputs
            model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs)
            model_evil_inputs = self.prepare_inputs_for_generation(evil_input_ids, **model_kwargs)
